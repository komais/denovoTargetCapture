...
setwd("~/Dropbox/GRP Data/GRP mammal paper/NN_analysis/DATA")
setwd("C:/Users/karowe/Dropbox/GRP Data/GRP mammal paper/NN_analysis/DATA/2014 input files")
...
## Checked 4/1: all files are in the same resolution

unqsites<-read.table("unq_aggsites_ebc_20120410.csv", header=T, sep=",");
...
YO_rareH_b1<-rep(NA,length(YO_points[,1]))
YO_rareH_b5<-rep(NA,length(YO_points[,1]))
YO_rareH_b6<-rep(NA,length(YO_points[,1]))
YO_rareH_b12<-rep(NA,length(YO_points[,1]))
...
for(i in 1:length(YO_avg_b1)) {
	b1_pts<-order(abs(b1_YO1-YO_points[i,"H_BIO_1"]))[1:(0.05*round(length(YOi)))] #top 5% hist_bio1 - index of cells, not the cell values
  YO_mr_b1[i]<-((length(which((abs(YO_pt_Hb1[i]-b1_YO1[b1_pts])/100)<1))/length(b1_pts))*100) #micro-refugia
	...
	YO_mr_b5[i]<-((length(which((abs(YO_pt_Hb5[i]-b5_YO1[b5_pts])/100)<1))/length(b5_pts))*100) #micro-refugia
  ...
  YO_mr_b6[i]<-((length(which((abs(YO_pt_Hb6[i]-b6_YO1[b6_pts])/100)<1))/length(b6_pts))*100) #micro-refugia
	...
	YO_mr_b12[i]<-((length(which((abs(YO_pt_Hb12[i]-b12_YO1[b12_pts])/1000)<10))/length(b12_pts))*100) #micro-refugia
	YO_avg_b12[i]<-mean(elev_YO[b12_pts],na.rm=T)
	}
elev_YO_pts<-extract(dem_YO,data.frame(YO_points$X,YO_points$Y))


#rare climates analysis - for the 5% closest climatically to the historical site value, what percentage of cells are within 1C or 10 mm
for(i in 1:length(YO_avg_b1)) {
  b1_pts<-order(abs(b1_YO0-YO_points[i,"H_BIO_1"]))[1:(0.05*round(length(YOi)))] #top 5% hist_bio1
#b1_pts - the 5% nearest cells climatically within historical era of the historical site value
YO_rareH_b1[i]<-((length(which((abs(YO_pt_Hb1[i]-b1_YO0[b1_pts])/100)<1)))/length(b1_pts))*100
b5_pts<-order(abs(b5_YO0-YO_points[i,"H_BIO_5"]))[1:(0.05*round(length(YOi)))] #top 5% hist_bio5
  YO_rareH_b5[i]<-((length(which((abs(YO_pt_Hb5[i]-b5_YO0[b5_pts])/100)<1)))/length(b5_pts))*100
b6_pts<-order(abs(b6_YO0-YO_points[i,"H_BIO_6"]))[1:(0.05*round(length(YOi)))] #top 5% hist_bio5
  YO_rareH_b6[i]<-((length(which((abs(YO_pt_Hb6[i]-b6_YO0[b6_pts])/100)<1)))/length(b6_pts))*100
b12_pts<-order(abs(b12_YO0-YO_points[i,"H_BIO_12"]))[1:(0.05*round(length(YOi)))] #top 5% hist_bio5
  YO_rareH_b12[i]<-((length(which((abs(YO_pt_Hb12[i]-b12_YO0[b12_pts])/1000)<10)))/length(b12_pts))*100
...
LA_rareH_b1<-rep(NA,length(LA_points[,1]))
LA_rareH_b5<-rep(NA,length(LA_points[,1]))
LA_rareH_b6<-rep(NA,length(LA_points[,1]))
LA_rareH_b12<-rep(NA,length(LA_points[,1]))

for(i in 1:length(LA_avg_b1)) {
  b1_pts<-order(abs(b1_LA1-LA_points[i,"H_BIO_1"]))[1:(0.05*round(length(LAi)))] #top 5% hist_bio1
  LA_mr_b1[i]<-((length(which((abs(LA_pt_Hb1[i]-b1_LA1[b1_pts])/100)<1))/length(b1_pts))*100) #micro-refugia
  ...
  LA_mr_b5[i]<-((length(which((abs(LA_pt_Hb5[i]-b5_LA1[b5_pts])/100)<1))/length(b5_pts))*100) #micro-refugia
  ...
  LA_mr_b6[i]<-((length(which((abs(LA_pt_Hb6[i]-b6_LA1[b6_pts])/100)<1))/length(b6_pts))*100) #micro-refugia
  ...
  LA_mr_b12[i]<-((length(which((abs(LA_pt_Hb12[i]-b12_LA1[b12_pts])/1000)<10))/length(b12_pts))*100) #micro-refugia
...
for(i in 1:length(LA_avg_b1)) {
  b1_pts<-order(abs(b1_LA0-LA_points[i,"H_BIO_1"]))[1:(0.05*round(length(LAi)))] #top 5% hist_bio1
  #b1_pts - the 5% nearest cells climatically within historical era of the historical site value
  LA_rareH_b1[i]<-((length(which((abs(LA_pt_Hb1[i]-b1_LA0[b1_pts])/100)<1)))/length(b1_pts))*100
  b5_pts<-order(abs(b5_LA0-LA_points[i,"H_BIO_5"]))[1:(0.05*round(length(LAi)))] #top 5% hist_bio5
  LA_rareH_b5[i]<-((length(which((abs(LA_pt_Hb5[i]-b5_LA0[b5_pts])/100)<1)))/length(b5_pts))*100
  b6_pts<-order(abs(b6_LA0-LA_points[i,"H_BIO_6"]))[1:(0.05*round(length(LAi)))] #top 5% hist_bio5
  LA_rareH_b6[i]<-((length(which((abs(LA_pt_Hb6[i]-b6_LA0[b6_pts])/100)<1)))/length(b6_pts))*100
  b12_pts<-order(abs(b12_LA0-LA_points[i,"H_BIO_12"]))[1:(0.05*round(length(LAi)))] #top 5% hist_bio5
  LA_rareH_b12[i]<-((length(which((abs(LA_pt_Hb12[i]-b12_LA0[b12_pts])/1000)<10)))/length(b12_pts))*100
...
SS_rareH_b1<-rep(NA,length(SS_points[,1]))
SS_rareH_b5<-rep(NA,length(SS_points[,1]))
SS_rareH_b6<-rep(NA,length(SS_points[,1]))
SS_rareH_b12<-rep(NA,length(SS_points[,1]))

for(i in 1:length(SS_avg_b1)) {
  b1_pts<-order(abs(b1_SS1-SS_points[i,"H_BIO_1"]))[1:(0.05*round(length(SSi)))] #top 5% hist_bio1
  SS_mr_b1[i]<-((length(which((abs(SS_pt_Hb1[i]-b1_SS1[b1_pts])/100)<1))/length(b1_pts))*100) #micro-refugia
  ...
  SS_mr_b5[i]<-((length(which((abs(SS_pt_Hb5[i]-b5_SS1[b5_pts])/100)<1))/length(b5_pts))*100) #micro-refugia
  ...
  SS_mr_b6[i]<-((length(which((abs(SS_pt_Hb6[i]-b6_SS1[b6_pts])/100)<1))/length(b6_pts))*100) #micro-refugia
  ...
  SS_mr_b12[i]<-((length(which((abs(SS_pt_Hb12[i]-b12_SS1[b12_pts])/1000)<10))/length(b12_pts))*100) #micro-refugia
  SS_avg_b12[i]<-mean(elev_SS[b12_pts],na.rm=T)
}
elev_SS_pts<-extract(dem_SS,data.frame(SS_points$X,SS_points$Y))

for(i in 1:length(SS_avg_b1)) {
  b1_pts<-order(abs(b1_SS0-SS_points[i,"H_BIO_1"]))[1:(0.05*round(length(SSi)))] #top 5% hist_bio1
  #b1_pts - the 5% nearest cells climatically within historical era of the historical site value
  SS_rareH_b1[i]<-((length(which((abs(SS_pt_Hb1[i]-b1_SS0[b1_pts])/100)<1)))/length(b1_pts))*100
  b5_pts<-order(abs(b5_SS0-SS_points[i,"H_BIO_5"]))[1:(0.05*round(length(SSi)))] #top 5% hist_bio5
  SS_rareH_b5[i]<-((length(which((abs(SS_pt_Hb5[i]-b5_SS0[b5_pts])/100)<1)))/length(b5_pts))*100
  b6_pts<-order(abs(b6_SS0-SS_points[i,"H_BIO_6"]))[1:(0.05*round(length(SSi)))] #top 5% hist_bio5
  SS_rareH_b6[i]<-((length(which((abs(SS_pt_Hb6[i]-b6_SS0[b6_pts])/100)<1)))/length(b6_pts))*100
  b12_pts<-order(abs(b12_SS0-SS_points[i,"H_BIO_12"]))[1:(0.05*round(length(SSi)))] #top 5% hist_bio5
  SS_rareH_b12[i]<-((length(which((abs(SS_pt_Hb12[i]-b12_SS0[b12_pts])/1000)<10)))/length(b12_pts))*100
...
neighbor_export$Crossera_comparecells_b1<-c(YO_mr_b1 ,LA_mr_b1,SS_mr_b1)
neighbor_export$Crossera_comparecells_b5<-c(YO_mr_b5 ,LA_mr_b5,SS_mr_b5)
neighbor_export$Crossera_comparecells_b6<-c(YO_mr_b6 ,LA_mr_b6,SS_mr_b6)
neighbor_export$Crossera_comparecells_b12<-c(YO_mr_b12 ,LA_mr_b12,SS_mr_b12)
neighbor_export$Hist_comparecells_b1<-c(YO_rareH_b1,LA_rareH_b1, SS_rareH_b1)
neighbor_export$Hist_comparecells_b5<-c(YO_rareH_b5,LA_rareH_b5, SS_rareH_b5)
neighbor_export$Hist_comparecells_b6<-c(YO_rareH_b6,LA_rareH_b6, SS_rareH_b6)
neighbor_export$Hist_comparecells_b12<-c(YO_rareH_b12,LA_rareH_b12, SS_rareH_b12)
...
write.table(neighbor_export,"rare_nn_modernandhistorical_climates_20140416_3.csv",sep=",")
